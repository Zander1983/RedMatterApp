...
FCS.prototype.setMaxes = function (databuf) {
  var maxArray = this.get$PnX("R");
  maxArray = generalHelper.removeNulls(maxArray);
  this.maxArray = maxArray;
};

FCS.prototype.getEvents = function (databuf) {
  return this.dataAsNumbers;
};

FCS.prototype.getEvent = function (params) {
...
FCS.prototype.getParams = function () {
  return generalHelper.removeNulls(this.get$PnX("N"));
};

FCS.prototype.getParamsAndScales = function () {
...
FCS.prototype.getParamNameFromIndex = function (params) {
  var index = params.index;

  return this.get("text", "$P" + (index + 1) + "N");
};

FCS.prototype.getIndexesOfParams = function (params) {
...
FCS.prototype.getParamLike = function (params) {
...
FCS.prototype.getParamNameHasSpillover = function (params) {
  var self = this;
...
FCS.prototype.getSpillover = function () {
...
FCS.prototype.parseSpillover = function (params) {
  var spillover;
  var self = this;
  ...
  if (spillover && spillover.length > 0) {
    ...
    if (determinant != 0) {
      ...
      } else if (Array.isArray(spilloverParams) && !isNaN(spilloverParams[0])) {
        // so '1, '2', '3'
...
var getFcsFromDataBuf = function (params) {
  ...
  return new Promise(function (resolve, reject) {
    var fcs = new FCS(options, params.databuf);
    if (fcs) {
      resolve(fcs);
    } else {
      reject(fcs);
