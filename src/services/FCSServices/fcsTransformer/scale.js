...
class Scale {
  ...
  scaleValueAccordingToFile(params) {
    var scaleType = params.scaleType;
    ...
    if (scaleType == "lin") {
      throw Error("Not implemented corner case");
      //return value;
  ...
  getNearestPower(v) {
    ...
    return (
      (v >= 0 || -1) * Math.pow(10, 1 + Math.floor(Math.log10(Math.abs(v))))
    );
  ...
  getIndexOfSpilloverParam(params) {
    var indexOfParam;
    if (Array.isArray(this.spilloverParams) && isNaN(this.spilloverParams[0])) {
  ...
  memoizedScaleValueAccordingToFile = memoize(this.scaleValueAccordingToFile, {
    length: 4,
  });

  adjustSpillover(params) {
    ...
    for (var i = 0; i < this.indexesOfSpilloverParams.length; i++) {
      if (this.invertedMatrix._data[i][indexOfSpilloverParam] != 0) {
        ...
        adjusted +=
          scaled * this.invertedMatrix._data[i][indexOfSpilloverParam];
      }
    }
