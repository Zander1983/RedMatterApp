// import FCSWriteStream from "./fcswritestream";
...
//   if (buffer) {
//     if (Buffer.isBuffer(buffer)) this.readBuffer(buffer);
//     else throw "only Buffers supported for now";
//   }
...
//   if (!(this instanceof FCS)) return new FCS(options);

//   if (options)
//     for (var prop in options)
//       if (options.hasOwnProperty(prop)) this.meta[prop] = options[prop];
...
//   // static usage is impossible currently, but in case we change the "classiness" in the future...
//   if (!(this instanceof FCS)) return new FCS(moreOptions, databuf);
...
//   var textSegment = databuf.toString(
//     encoding,
...
//     var analysisSegment = databuf.toString(
//       encoding,
...
//   var result = [];
//   result[0] = null;
//   for (var p = 1; p <= this.meta.$PAR; p++) result[p] = this.text["$P" + p + x];
...
//   "use strict";
//   if (this.dataAsNumbers) return this.dataAsNumbers[idx - 1];
//   else return null;
// };
...
//   "use strict";
//   if (this.dataAsStrings) return this.dataAsStrings[idx - 1];
//   else return null;
// };
...
//   else throw "cannot handle $BYTEORD= " + this.text.$BYTEORD;
