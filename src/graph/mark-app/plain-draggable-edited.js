...
function getBBox(element, getPaddingBox) {
  ...
  if (getPaddingBox) {
    ...
    // this seems to set heigh ot the wrokspace wrapper
  }
...
function initTranslate(props) {
  ...
  if (!props.orgStyle) {
    ...
    RESTORE_PROPS.forEach(function (prop) {
      ...
      //   if (prop && prop == "height" && props.orgStyle[prop] && props.orgStyle[prop].length > 0) {
      //     console.log("setting " + prop + " to " + props.orgStyle[prop]);
      //   }

      //   if (prop && prop == "width" && props.orgStyle[prop] && props.orgStyle[prop].length > 0) {
      //     console.log("setting " + prop + " to " + props.orgStyle[prop]);
      //   }
  ...
  ['width', 'height'].forEach(function (prop) {
    if (newBBox[prop] !== orgSize[prop]) {
      ...
      if (newBBox[prop] !== orgSize[prop]) {
        // Retry
        // if (prop && prop == "height") {
        //   console.log("setting " + prop + " to " + (orgSize[prop] - (newBBox[prop] - orgSize[prop])) + "px");
        // }
...
function initLeftTop(props) {
  ...
  if (!props.orgStyle) {
    ...
    RESTORE_PROPS.forEach(function (prop) {
      ...
      // if (props.lastStyle[prop] == null || elementStyle[prop] === props.lastStyle[prop]) {
      //   if (prop && prop == "height" && props.orgStyle[prop] && props.orgStyle[prop].length > 0) {
      //     console.log("initLeftTop: setting " + prop + " to " + props.orgStyle[prop]);
      //   }
  ...
  ['width', 'height'].forEach(function (prop) {
    if (newBBox[prop] !== orgSize[prop]) {
      // Ignore `box-sizing`
      // if (prop && prop == "height") {
      //   console.log("initLeftTop 1: setting " + prop + " to " +  orgSize[prop] + 'px');
      // }
      // elementStyle[prop] = orgSize[prop] + 'px';
      newBBox = getBBox(element);
      ...
      if (newBBox[prop] !== orgSize[prop]) {
        // Retry
        // if (prop && prop == "height") {
        //   console.log("initLeftTop 2: setting " + prop + " to " +  (orgSize[prop] - (newBBox[prop] - orgSize[prop])) + 'px');
        // }
        // elementStyle[prop] = orgSize[prop] - (newBBox[prop] - orgSize[prop]) + 'px';
      }
    }
