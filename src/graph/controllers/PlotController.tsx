...
/*
  This is supposed to be inserted into the PlotView class (which is now 
                                                           PlotComponent)
*/
const PlotController = (props: {}): null => {
  return null;
};
...
class PlotDataLEGACY extends ObserversFunctionality {
  readonly id: string;
  ...
  setState(state: PlotControllerState) {
  ...
  createSubpop(inverse: boolean = false) {
    ...
    const newPlotController = new PlotController();
    newPlotController.setState(this.getState());
    newPlotController.populationModel =
      this.populationModel.createSubpop(inverse);
    newPlotController.parentPlotId = this.id;
    newPlotController.updateGateObservers();
    newPlotController.positions = {
      x: -1,
      y: -1,
    };
    newPlotController.dimensions = {
      w: 10,
      h: 12,
    };
    newPlotController.parentPlotId = this.id;
    dataManager.redrawPlotIds.push(this.id);
    return dataManager.addNewPlotToWorkspace(newPlotController);
  }
