...
export default class ScatterPlot {
    ...
    createRangeArray(axis: 'x' | 'y'): Array<string> {
        ...
        const rangeSize = axis === 'x' ? 
            this.xRange[1]-this.xRange[0] : this.yRange[1]-this.yRange[0]
        const rangeMin = axis === 'x' ? this.xRange[0] : this.yRange[0]
        const lineCount = Math.round(plotSize / 100)
        return Array(lineCount).map(
                (e, i) => (rangeSize*i/lineCount + rangeMin).toString()
            )
    }

    findRangeBoundries(axis: 'x' | 'y'): [number, number] {
        const iaxis = axis === 'x' ? 0 : 1
        let min = this.data[0][iaxis], max = this.data[0][iaxis]
        this.data.forEach(e => {
    ...
    draw(context: Context, frameCount: number) {
        ...
        this.data.forEach((point: [number, number]) => {
            plotGraph.addPoint(
                point[this.xAxis.key],
                point[this.yAxis.key]
            )
        });
