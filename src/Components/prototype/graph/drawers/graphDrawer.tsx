...
export default class GraphDrawer extends Drawer {
    ...
    constructor({ context, x1, y1, x2, y2, ibx, iex, iby, iey }:
    ...
    drawPlotGraph(): PlotGraph {
        ...
        for (let i = 0; i < 10; i++) {
            const height = (Math.abs(this.y1 - this.y2) / 10) * i
        ...
        // Vertical plot lines
        for (let i = 1; i <= 10; i++) {
            const width = (Math.abs(this.x1 - this.x2) / 10) * i 
        ...
        return ((parent) => {
            return {
                addPoint: (x: number, y: number, color: string = '#000') => {
                    if (x < parent.ibx || x > parent.iex)
                        return
                    if (y < parent.iby || y > parent.iey)
                        return
                ...
                addLine: (pa: [number, number], pb: [number, number], color: string = '#000') => {
