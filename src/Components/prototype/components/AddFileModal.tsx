...
function AddFileModal(props: {
    ...
    const generateRandomFile = (maxPoints: number, l: number, r: number) => {
        ...
        <Modal
            ...
            <div className={classes.fileSelectModal}>
                ...
                <p style={{
                    ...
                    The prototype still doesn't allow for uploading files or saving
                    them, but here we have a selection of 3 real fcs files for you to
                    play around!
                </p>

                <p><b>Click on the file you want to open:</b></p>
                ...
                <div style={{
                    ...
                    {
                        files.map((e, i) => {
                            const divider = i == files.length - 1 ? null :
                                (<Divider className={classes.fileSelectDivider} />)
                            
                            const payload = (
                                    <div
                                        key={i.toString()+e.title}>
                                        <div
                                            onMouseEnter={() => setOnHover(i)}
                                            onMouseLeave={() => setOnHover(-1)}
                                            className={onHover == i ? classes.fileSelectFileContainerHover : classes.fileSelectFileContainer}
                                            onClick={() => {
                                                addFile(i)
                                                props.closeCall.f(props.closeCall.ref)
                                            }}>
                                            <p><b>Title:</b> <a style={{color: '#777', fontSize: 16, fontFamily: 'Courier New'}}>{e.title}</a></p>
                                            <p><b>Last Modified:</b> {e.lastModified}</p>
                                            <p><b>Information:</b> {e.information}</p>
                                        </div>
                                        {divider}
                                    </div>
                                )
